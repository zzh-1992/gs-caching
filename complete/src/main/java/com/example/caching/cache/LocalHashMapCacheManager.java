...
/**
 * 拷贝源码ConcurrentMapCacheManager,仅仅替换了cacheMap的类型(后续优化)
 *
 * {@link CacheManager} implementation that lazily builds {@link HashMapCache}
 ...
 * @since 3.1
 */
public class LocalHashMapCacheManager implements CacheManager, BeanClassLoaderAware {

    private final HashMap<String, Cache> cacheMap = new HashMap<>(16);
    ...
    /**
     * Construct a dynamic HashMapCacheManager,
     * lazily creating cache instances as they are being requested.
     */
    public LocalHashMapCacheManager() {
    }

    /**
     * Construct a static HashMapCacheManager,
     * managing caches for the specified cache names only.
     */
    public LocalHashMapCacheManager(String... cacheNames) {
    ...
    public void setCacheNames(@Nullable Collection<String> cacheNames) {
        if (cacheNames != null) {
            for (String name : cacheNames) {
                this.cacheMap.put(name, createHashMapCache(name));
            }
            this.dynamic = false;
        } else {
            this.dynamic = true;
    ...
    private void recreateCaches() {
        for (Map.Entry<String, Cache> entry : this.cacheMap.entrySet()) {
            entry.setValue(createHashMapCache(entry.getKey()));
        }
    }

    /**
     * Create a new HashMapCache instance for the specified cache name.
     *
     * @param name the name of the cache
     * @return the HashMapCache (or a decorator thereof)
     */
    protected Cache createHashMapCache(String name) {
        SerializationDelegate actualSerialization = (isStoreByValue() ? this.serialization : null);
        return new LocalHashMapCache(name, new HashMap<>(256), isAllowNullValues(), actualSerialization);
    }
